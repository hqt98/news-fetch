...
class Newspaper:
    def __init__(self, uri: str):
        ...
        self.article = catch(
            ...
            lambda: (
                ...
                if article.text != None
                    ...
                    news_article(newsplease.maintext)
                    if newsplease.maintext != None
                    else "None"
                )
            ),
        )
        ...
        self.authors = catch(
            ...
            lambda: (
                ...
                if len(newsplease.authors) != 0
                    ...
                    else unicode([author(soup)]) if author(soup) != None else ["None"]
                )
            ),
        )
        ...
        """
        self.date_publish = catch(
            "None",
            lambda: (
                str(newsplease.date_publish)
                if str(newsplease.date_publish) != "None"
                else (
                    article.meta_data["article"]["published_time"]
                    if article.meta_data["article"]["published_time"] != None
                    else date(soup) if date(soup) != None else "None"
                )
            ),
        )
        ...
        self.date_download = catch("None", lambda: str(newsplease.date_download))
        ...
        """
        self.publication = catch(
            "None",
            lambda: (
                article.meta_data["og"]["site_name"]
        ...
        """
        meta_check = any(
            word in "section" or "category" for word in list(article.meta_data.keys())
        )
        self.category = catch(
            "None",
            lambda: (
                article.meta_data["category"]
                if meta_check == True and article.meta_data["category"] != {}
                else (
                    article.meta_data["section"]
                    if meta_check == True and article.meta_data["section"] != {}
                    else (
                        article.meta_data["article"]["section"]
                        if meta_check == True
                        and article.meta_data["article"]["section"] != {}
                        else category(soup) if category(soup) != None else "None"
                    )
                )
            ),
        )
        ...
        self.headline = catch(
            ...
            lambda: (
                ...
                else unicode(newsplease.title) if newsplease.title != None else "None"
            ),
        )
        ...
        self.description = catch(
            ...
            lambda: (
                ...
                if article.meta_description != ""
                    ...
                    news_article(article.meta_data["description"])
                    if article.meta_data["description"] != {}
                    else (
                        news_article(newsplease.description)
                        if newsplease.description != None
                        else None
                    )
                )
            ),
